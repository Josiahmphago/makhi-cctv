...
export async function createCheckoutOzow(args: CreateOzowArgs): Promise<{
  paymentUrl: string;
}> {
  ...
  if (!siteCode || !privateKey) {
    ...
    throw new Error(
      "Ozow credentials missing. Set OZOW_SITE_CODE and OZOW_PRIVATE_KEY in .env"
    );
  }

  // TODO: Implement Ozow checksum + request here.
  // For now we simulate returning a hosted payment page URL.
  const fakeUrl = `${baseUrl.replace(/\/+$/, "")}/pay/${encodeURIComponent(
    reference
  )}?amount=${amountZAR.toFixed(2)}&success=${encodeURIComponent(
    successUrl
  )}&cancel=${encodeURIComponent(cancelUrl)}&notify=${encodeURIComponent(
    notifyUrl
  )}`;
